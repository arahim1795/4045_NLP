...
def get_noun_pairs_index(words_obj):
    ...
    for i in range(get_length_of_words_obj(words_obj)):
        if predicted_pos[i] in INTERESTED_NOUN_POS and predicted_dependencies[i] == "conj":
...
def get_adjective_pairs_index(words_obj):
    ...
    adj_pairs_index = {}
    for i in range(get_length_of_words_obj(words_obj)):
        if predicted_pos[i] in INTERESTED_ADJ_POS and predicted_dependencies[i] == "conj":
            predicted_heads_index = predicted_heads[i] - 1
...
def get_noun_adjective_pairs(words_obj, noun_pairs, adjective_pairs):
    ...
    for i in range(get_length_of_words_obj(words_obj)):
        if (predicted_pos[i] in INTERESTED_ADJ_POS and predicted_pos_of_heads[i] in INTERESTED_NOUN_POS) or \
            (predicted_pos[i] in INTERESTED_NOUN_POS and predicted_pos_of_heads[i] in INTERESTED_ADJ_POS):
            predicted_heads_index = predicted_heads[i] - 1
