...
def get_dependency_tree_from_sentence(sentence):
    ...
    dependency_tree = predictor.predict(sentence=sentence)
...
def create_graph_from_dependency_tree_dict(dependency_tree_dict):
    ...
    for i in range(len(dependency_tree_dict["words"])):
        ...
        if pos[i] in INTERESTED_NOUN_POS:
            ...
            if pos[head_index] in INTERESTED_ADJ_POS:
                ...
                pos[head_index] in INTERESTED_NOUN_POS
                and predicted_dependencies[i] in CONJUNCTION
            ):
        ...
        if pos[i] in INTERESTED_ADJ_POS:
            ...
            # add_edge_from_first_node_to_second_node(G, ROOT_NODE_INDEX, noun_index)
        if (
            pos[i] in INTERESTED_ADVERB_POS
            and predicted_dependencies[i] in CORRECT_ADVERB_DEPENDENCIES
        ):
            head_index = predicted_heads[i] - 1
            if pos[head_index] in INTERESTED_ADJ_POS:
                add_edge_from_first_node_to_second_node(G, head_index, i)
    for i in range(len(dependency_tree_dict["words"])):
        if pos[i] in INTERESTED_ADJ_POS:
            ...
            if (
                G.has_node(i)
                and len(list(G.predecessors(i))) > 0
                and pos[list(G.predecessors(i))[0]] in INTERESTED_NOUN_POS
            ):
                continue
            while not (
                G.has_node(head_index)
                and len(list(G.predecessors(head_index))) > 0
                and pos[list(G.predecessors(head_index))[0]] in INTERESTED_NOUN_POS
            ):
        ...
        for adjective_index in adjective_indexes:
            ...
            if len(adverb_indexes) == 0:
                ...
                for temp_noun_index in temp_noun_indexes:
                    temp_noun_groupings = get_noun_groupings(
                        dependency_tree_dict, temp_noun_index
                    )
                    noun_adjective_pairs.append(
                        (temp_noun_groupings, words[adjective_index])
                    )
            else:
                adverbs = []
                adverb_indexes.sort()
                ...
                for temp_noun_index in temp_noun_indexes:
                    ...
                    noun_adjective_pairs.append(
                        (temp_noun_groupings, adjective_groupings)
                    )
    return noun_adjective_pairs
